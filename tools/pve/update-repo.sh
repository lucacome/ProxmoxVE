...
function update_container() {
  ...
  if [[ "$os" == "ubuntu" || "$os" == "debian" ]]; then
    ...
    if pct exec "$container" -- [ -e /usr/bin/update ]; then
      if pct exec "$container" -- grep -q "lucacome/ProxmoxVE" /usr/bin/update; then
...
header_info
echo -e "${GN}The process is complete. The repositories have been switched to lucacome/ProxmoxVE.${CL}\n"
